...
public class Solution extends Stack<Candidate>
{
	...
	private boolean bordersCard(int row, int column, char cardChar){
		if(board[row][column-1].getCardChar() == cardChar){
			return false;
		}else if(board[row][column+1].getCardChar() == cardChar){
			return false;
		}else if(board[row-1][column].getCardChar() == cardChar){
			return false;
		}else if(board[row+1][column].getCardChar() == cardChar){
			return false;
		}else {
	...
	public boolean fits(Candidate candidate){ 
		Candidate lastCardAdded = this.peek();
		if(lastCardAdded.getCardChar() == candidate.getCardChar()){
			return false;
		}
	    return true;
    }
